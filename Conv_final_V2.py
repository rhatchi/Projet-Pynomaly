...
def dataset_stream(set_files, scaler, sr=16000, n_mels=128, frames=5, n_fft=1024, hop_length=512, power=1.0):
    """
...
def feature_selection_train(path_csv, machinetype):
    ...
    train_data_stream = dataset_stream(train_data, scaler)
    
    return train_data_stream, scaler
...
def model_conv_training(machinetype,train_data_stream):
    ...
    history = model_conv.fit(train_data_stream, train_data_stream,
                batch_size = 64,
                epochs = 3,
...
def f1scores(error_train_data,error_test_data,y_true,machinetype,machineID):
    ...
    np.save("ConvAE_F1_1"+machinetype+'_'+str(machineID),np.asarray(f1_1))
    np.save("ConvAE_F1_0"+machinetype+'_'+str(machineID),np.asarray(f1_0))
    np.save("ConvAE_F1_somme"+machinetype+'_'+str(machineID),np.asarray(somme))
...
def model_evaluation(path_csv, machinetype, model_conv, scaler):
    ...
    for machineID in IDs[machinetype] :
        ...
        pred_test_data_byID = model_conv.predict(test_data_stream_byID)
