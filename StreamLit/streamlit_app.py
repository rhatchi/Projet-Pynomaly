...
if active_page == pages[0]:
    ...
    st.write('Le dataframe du projet contenant toutes les informations élémentaires :')
    st.write(df)  
    
    """
    * Path : le chemin du fichier audio
    * Machine_Type : le type de machine
    * Dataset : le jeu de données (train ou test)
    * Status : le label cible (normal ou anormal)
    * Machine_ID : l'ID de la machine
    * Sample_ID : l'ID de la prise de son
    * freq_echantillonnage : la fréquence d'échantillonnage
    * nb_echantillon : le nombre d'échantillons
    * audio : le fichier numérisé en un vecteur array 
    """
...
if active_page == pages[1]:
    st.title("Analyse statistique du dataframe")
...
if active_page == pages[2]:
    ...
    """
    * Modèles classiques de machine learning inutilisables dans notre situation : résultats peu significatifs.
    * Objectif : détecter les anomalies --> apprentissage non-supervisé. 
    * Modèles Auto-encoders (ou encoder/decoder) adaptés à notre projet. 
    * Deux modèles AE utilisés : le Dense et le Convolutif.
    
    
    """
    
    st.subheader('Modèle Dense Auto-encoder :')
    FeatureDense= plt.imread("/Users/romeo/FormationDataScientist/ProjetPynomaly/ModelDense.png")   
    st.image(FeatureDense)

    st.write('\n\n')
    st.subheader('Modèle Convolutif Auto-encoder :')    
    FeatureConv= plt.imread("/Users/romeo/FormationDataScientist/ProjetPynomaly/ModelConv.png")   
...
if active_page == pages[3]:
    ...
    FeatureDense= plt.imread("/Users/romeo/FormationDataScientist/ProjetPynomaly/FeatureDense.jpg")   
    st.image(FeatureDense)

    st.write('\n\n')
    st.subheader('Sélection de features pour le modèle Convolutif Auto-encoder :')    
    FeatureConv= plt.imread("/Users/romeo/FormationDataScientist/ProjetPynomaly/FeatureConvolutif.jpg")   
    st.image(FeatureConv)
...
if active_page == pages[4]:
    ...
    if model == 'dense':
        df = df.sample(n=100, random_state = 13)
        st.write(df)

        df.Path = df.Path.apply(lambda x : '/Users/romeo/FormationDataScientist/ProjetPynomaly/'+x)
        model_trained = tf.keras.models.load_model('/Users/romeo/FormationDataScientist/ProjetPynomaly/trained_models/denseAE_' + machinetype + '.hdf5', compile=False)
        ...
        df.Path = df.Path.apply(lambda x : '/Users/romeo/FormationDataScientist/ProjetPynomaly/'+x)
        scaler = load('/Users/romeo/FormationDataScientist/ProjetPynomaly/scaler/std_scaler_'+ machinetype +'.bin')
        model_trained = tf.keras.models.load_model('/Users/romeo/FormationDataScientist/ProjetPynomaly/trained_models/convAE_' + machinetype + '.hdf5', compile=False)
    ...
    if seuil :
        ...
        st.write('La matrice de confusion avec ce seuil est :')
        st.write(DF.matrice_confusion(y_true, y_pred))
        st.write('\n\n Et le rapport de classification de ces prédictions est :')
...
if active_page == pages[5]:
    ...
    for machinetype in machine_types:
        evaluation_data_file = "/Users/romeo/FormationDataScientist/ProjetPynomaly/results/"+machinetype+"/"
