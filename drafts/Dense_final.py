...
def feature_selection(machinetype):
    ...
    test_data_stream = dataset_stream(test_data)
    
    # Création du vecteur label cible
    y_true = test_data['Status'].replace(['normal', 'anomaly'], [0,1])
    
    # Sauvegarde des data steams
    np.save("TrainPreprocessedData_DenseAE_"+machinetype,train_data_stream)
    np.save("TestPreprocessedData_DenseAE_"+machinetype,test_data_stream)
    
    return train_data,test_data,train_data_stream,test_data_stream,y_true
...
def model_evaluation(machinetype,model_dense,
                     train_data,test_data,
                     train_data_stream,test_data_stream,
                     train_data_len,test_data_len,
                     y_true,nb_extract = 532):
    
    if machinetype == "ToyCar" : nb_extract = 588
    #Erreurs sur l'entraînement
    pred_train_data = model_dense.predict(train_data_stream)
    error_train_data = errors(train_data_stream, pred_train_data, train_data_len,nb_extract)
    ...
    pred_test_data = model_dense.predict(test_data_stream)
    error_test_data = errors(test_data_stream, pred_test_data, test_data_len,nb_extract)
